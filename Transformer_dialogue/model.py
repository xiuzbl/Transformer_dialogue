...
class Transformer(nn.Module):
    ...
    def forward(self,  # type: ignore
        ...
        #print("encoder_input_length:", encoder_input_length)
        #print("encoder_input_length.max():", encoder_input_length.max())
        #print("encoder_input_length.max()arange:", torch.arange(encoder_input_length.max(), dtype=torch.long,device=encoder_input.device))
        #print("encoder_input_length_max:", encoder_input_length_max)
        ...
        if decoder_input is not None and labels is not None:
            ...
            # For training
            #print("self.word_embedder.embedding:", self.word_embedder.embedding)
            #print("output_layer.weight:", self.decoder.output_layer.weight)
            #print("self.pos_embedder.embedding:", self.pos_embedder.embedding)
            outputs = self.decoder(
                memory=encoder_output,
                memory_sequence_length=encoder_input_length,
                inputs=decoder_input,
                decoding_strategy="train_greedy",
                my_emb_fn=self._embedding_fn,
            )
